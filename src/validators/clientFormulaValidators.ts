...
export const clientFormulaSectionSchema = z.object({
  type: clientFormulaSectionTypeSchema,
...
const formulaBaseSchema = z.object({
  ...
  title: z.string().trim().min(1).max(160), formula_date: isoDateSchema,
  service_name_snapshot: z.string().trim().min(1).max(160),
...
export const listClientFormulasQuerySchema = z.object({ cursor: z.string().min(1).max(1000).optional(), limit: z.coerce.number().int().min(1).max(100).default(25) });
