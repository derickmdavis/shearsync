...
create table if not exists public.users (
  ...
  deletion_status text not null default 'active' check (deletion_status in ('active', 'pending', 'processing', 'failed')),
...
create index if not exists users_deletion_status_idx on public.users (deletion_status) where deletion_status <> 'active';
...
create table if not exists public.client_communication_preferences (
  ...
  sms_last_consent_event_id uuid,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
alter table public.client_communication_preferences
  add constraint client_communication_preferences_sms_last_consent_event_fkey
  foreign key (sms_last_consent_event_id)
...
create table if not exists public.client_formula_images (
  ...
  client_id uuid not null references public.clients(id) on delete cascade,
  formula_id uuid not null references public.client_formulas(id) on delete cascade,
  bucket text not null default 'appointment-images' check (bucket = 'appointment-images'),
  ...
  thumbnail_size_bytes bigint check (thumbnail_size_bytes is null or (thumbnail_size_bytes > 0 and thumbnail_size_bytes <= 307200)),
  width integer check (width is null or (width > 0 and width <= 1600)),
  height integer check (height is null or (height > 0 and height <= 1600)),
  thumbnail_width integer check (thumbnail_width is null or (thumbnail_width > 0 and thumbnail_width <= 400)),
  thumbnail_height integer check (thumbnail_height is null or (thumbnail_height > 0 and thumbnail_height <= 400)),
  image_role text not null default 'formula' check (image_role in ('formula', 'reference', 'inspiration', 'general')),
  caption text check (caption is null or char_length(caption) <= 1000),
  upload_status text not null default 'pending' check (upload_status in ('pending', 'ready', 'failed', 'expired')),
  upload_expires_at timestamptz,
  finalized_at timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create table if not exists public.client_formula_photos (
  id uuid primary key default gen_random_uuid(),
  formula_id uuid not null references public.client_formulas(id) on delete cascade,
  formula_image_id uuid references public.client_formula_images(id) on delete cascade,
  appointment_image_id uuid references public.appointment_images(id) on delete cascade,
  photo_type text,
  sort_order integer not null default 0 check (sort_order >= 0),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint client_formula_photos_exactly_one_image_check
    check ((formula_image_id is not null)::integer + (appointment_image_id is not null)::integer = 1),
  constraint client_formula_photos_formula_image_unique unique (formula_id, formula_image_id),
  constraint client_formula_photos_appointment_image_unique unique (formula_id, appointment_image_id)
);
...
begin
  ...
  insert into public.client_communication_preferences (user_id, client_id, phone, phone_normalized)
  values (p_user_id, p_client_id, p_phone, p_phone_normalized)
  ...
  if p_action = 'opt_in' then
    ...
    update public.client_communication_preferences set
      ...
      sms_opted_in_at = now(), sms_opt_in_source = p_source, sms_opt_in_text = p_consent_text,
      sms_opted_out_at = null, sms_opt_out_source = null, updated_at = now()
    ...
    update public.client_communication_preferences set
      ...
      sms_marketing_enabled = false, sms_rebooking_enabled = false,
      sms_opted_out_at = now(), sms_opt_out_source = p_source, updated_at = now()
    where id = preference.id returning * into preference;
    event_type := 'opted_out';
  else
    if preference.sms_opted_in_at is null or preference.opted_out_all_sms then raise exception 'sms_explicit_opt_in_required'; end if;
    update public.client_communication_preferences set
...
insert into public.insight_snapshot_configurations (id, configuration_version, is_active, enabled, pages, rollout_percentage, updated_by)
select '20260720-0000-4000-8000-000000000001'::uuid, 1, true, true, '[{"id":"business_performance","title":"Business Performance","layout":"grid_2x2","period_behavior":"selected_period","enabled":true,"metrics":[{"metric_id":"booked_revenue","enabled":true},{"metric_id":"appointments_booked","enabled":true},{"metric_id":"rebooking_rate","enabled":true},{"metric_id":"average_ticket","enabled":true}]}]'::jsonb, 100, 'system:initial-insights-configuration'
...
-- Account deletion request creation and immediate shutdown are deliberately one
-- transaction so a retry cannot observe a partially disabled account.
...
begin
  ...
  where user_id = p_user_id and status in ('pending', 'processing', 'failed_retryable')
  order by requested_at desc limit 1 for update;
  if found then
    v_duplicate := true;
  else
    insert into public.account_deletion_requests (user_id, status, reason, client_request_id, requested_at, scheduled_deletion_at, created_ip_hash, created_user_agent)
    values (p_user_id, 'pending', p_reason, p_client_request_id, v_now, p_scheduled_deletion_at, p_created_ip_hash, p_created_user_agent)
  ...
  update public.rebook_nudges set status = 'cancelled', cancelled_at = v_now, cancelled_reason = 'account_deletion_requested' where user_id = p_user_id and status in ('pending_approval', 'queued', 'sending', 'failed');
  update public.birthday_reminders set status = 'cancelled', cancelled_at = v_now, cancelled_reason = 'account_deletion_requested' where user_id = p_user_id and status in ('queued', 'sending', 'failed');
  update public.appointment_email_events set status = 'skipped', error = 'account_deletion_requested' where user_id = p_user_id and status in ('queued', 'sending', 'failed');
  update public.sms_messages set status = 'cancelled', error_code = 'account_deletion_requested', error_message = 'Account deletion requested' where user_id = p_user_id and status in ('queued', 'sending', 'failed');
  update public.appointment_sms_confirmation_jobs set status = 'skipped', error_code = 'account_deletion_requested', error_message = 'Account deletion requested' where user_id = p_user_id and status in ('pending', 'queued');
  update public.thank_you_emails set status = 'cancelled', cancelled_at = v_now, cancelled_reason = 'account_deletion_requested' where user_id = p_user_id and status in ('pending_approval', 'queued', 'sending', 'failed');
  update public.campaigns set status = 'cancelled', cancelled_at = v_now, cancelled_reason = 'account_deletion_requested' where user_id = p_user_id and status in ('draft', 'scheduled');
  update public.campaign_runs set status = 'cancelled', cancelled_at = v_now where user_id = p_user_id and status in ('draft', 'scheduled', 'queued');
  update public.campaign_recipients set status = 'cancelled', cancelled_at = v_now, error_code = 'account_deletion_requested' where user_id = p_user_id and status in ('pending', 'queued', 'failed');
  insert into public.account_deletion_audit_events (request_id, user_id, event_type, metadata)
  values (v_request.id, p_user_id, case when v_duplicate then 'duplicate_request' else 'requested' end, jsonb_build_object('clientRequestId', p_client_request_id, 'authSource', p_auth_source));
